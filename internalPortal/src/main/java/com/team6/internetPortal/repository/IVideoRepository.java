...
//	public List<Video> findAllVideoByCategory(int id);
	...
	@Modifying
	@Transactional
	@Query("update Video db set db.title=:title,db.description=:description, db.createdOn=:created_on, db.lastModifiedOn=:last_modified_on, db.category=:category, db.status=:status where db.id=:id")
	public int update(@Param("id") Long id,@Param("title") String title,@Param("description") String description, 
			 @Param("created_on") Date created_on,@Param("last_modified_on") Date last_modified_on, @Param("category") Category category, @Param("status") status status);
